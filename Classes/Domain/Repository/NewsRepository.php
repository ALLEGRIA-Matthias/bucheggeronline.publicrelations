...
class NewsRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
  ...
  public function findByProperty($property, $uid)
  {
    $query = $this->createQuery();
  ...
  public function findLatest($limit = 10)
  {
    ...
    $query = $this->createQuery();
    $query->matching(
      $query->lessThanOrEqual('date', date('Y-m-d', time()))
    );
    $query->setOrderings(["date" => \TYPO3\CMS\Extbase\Persistence\QueryInterface::ORDER_DESCENDING])->setLimit($limit);
